...
if __name__ == "__main__":
    ...
    deepsdf.load_state_dict(torch.load('checkpoints/checkpoint_e30.pth', map_location=device))

    embedding = LatentEmbedding(NUM_MESHES).to(device)
    embedding.load_state_dict(torch.load('checkpoints/checkpoint_e30_emb.pth', map_location=device))

    # First output all the raw meshes from the latent index
    for mesh_name in MESHES:
        latent = embedding(torch.tensor(MESHES[mesh_name]).to(device)).detach().cpu()
    ...
    for mesh_name_a, mesh_name_b, mesh_name_c in ARITHMETIC:
        latent_a = embedding(torch.tensor(MESHES[mesh_name_a]).to(device)).detach().cpu()
        latent_b = embedding(torch.tensor(MESHES[mesh_name_b]).to(device)).detach().cpu()
        latent_c = embedding(torch.tensor(MESHES[mesh_name_c]).to(device)).detach().cpu()
        latent = (latent_a + latent_b + latent_c) / 3
        mesh_name = '_'.join([mesh_name_a[:3], mesh_name_b[:3], mesh_name_c[:3]])
        mesh = utils.deepsdf_to_mesh(deepsdf, latent, eps=0.05, device=device,
